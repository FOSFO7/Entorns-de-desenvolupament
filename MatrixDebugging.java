	...
	public static void main(String[] args) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                transposedMatrix[j][((matrix.length-1)-i)] = matrix[i][j]; // Error: El índice i y j están invertidos
            }
        }
        ...
        try {
            System.out.println("\nAccediendo al elemento en [3][0]: " + matrix[2][0]);
        ...
        try {
            ...
            for (int i = 0; i < matrix.length; i++) {  // Error: índice fuera de límites // al quitar los iguales no accedemos a un sitio del array 
